...
val UniversalWizardButtons = partialState {
    onButton("Restart", color = Color.Red, section = Section.LEFT) { goto(Init) }
    onButton("Stop speaking", color = Color.Red, section = Section.LEFT) { furhat.stopSpeaking() }
}
...
val TestButtons = partialState {
    ...
    onButton("Nap", color = Color.Blue, section = Section.RIGHT) { goto(Nap) }
    onButton("Deepsleep", color = Color.Blue, section = Section.RIGHT) { goto(DeepSleep) }
    onButton("Wake up", color = Color.Yellow, section = Section.RIGHT) { furhat.wakeUp() }
    onButton("Fall asleep", color = Color.Yellow, section = Section.RIGHT) { furhat.fallASleep() }
    onButton("Idle", color = Color.Yellow, section = Section.RIGHT) { furhat.beIdle() }
    onButton("Be active", color = Color.Yellow, section = Section.RIGHT) { furhat.beActive() }
    onButton("Set furhat persona", color = Color.Yellow, section = Section.RIGHT) { activate(furhatPersona) }
}